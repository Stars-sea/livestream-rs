//! MinIO persistence handler — listens for segment completion events
//! and uploads TS segments and playlists to MinIO.
...
use livestream_transport::dispatcher::{self, SessionEvent};
...
impl SegmentPersistenceHandler {
    ...
    fn spawn_upload_listener(minio_client: PersistenceClient) {
        tokio::spawn(async move {
            Self::run_upload_listener(minio_client).await;
        });
    }

    async fn run_upload_listener(minio_client: PersistenceClient) {
        let mut events = dispatcher::INSTANCE.subscribe_global();

        while let Some(event) = events.recv().await {
            Self::handle_persistence_event(&minio_client, event).await;
        }
    }

    async fn handle_persistence_event(minio_client: &PersistenceClient, event: SessionEvent) {
        match event {
            ...
            SessionEvent::PlaylistUpdated {
                ...
                if let Err(e) = Self::upload_playlist(minio_client, &live_id, path, is_final).await
                {
    ...
    async fn upload_with_retry(
        ...
        let backoff_ms = [200, 400, 800, 1600];
        ...
        // Final attempt
        minio_client.upload_file(object_key, file_path).await
    }
