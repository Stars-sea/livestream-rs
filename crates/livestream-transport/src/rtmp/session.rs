...
impl SessionGuard {
    ...
    async fn handle_connect_event(
        ...
        match event {
            ServerSessionEvent::ConnectionRequested {
                ...
                self.handle_connection_requested(request_id, app_name, app_and_stream_re, ct)
                    .await
            }
            ...
            ServerSessionEvent::PublishStreamRequested {
                ...
                let stream_key =
                    Self::extract_stream_key(app_and_stream_re, &stream_key, &app_name);
    ...
    async fn handle_connection_requested(
        &mut self,
        request_id: u32,
        app_name: String,
        app_and_stream_re: &Regex,
        ct: &CancellationToken,
    ) -> Result<Option<HandlerBuilder>> {
    ...
    fn extract_stream_key(re: &Regex, stream_key: &str, app_name: &str) -> String {
        re.captures(stream_key)
            .and_then(|caps| {
                caps.name("stream_key")
                    .or_else(|| caps.name("app"))
                    .map(|m| m.as_str().to_string())
            ...
            .or_else(|| {
                re.captures(app_name).and_then(|caps| {
                    caps.name("stream_key")
                        .or_else(|| caps.name("app"))
                        .map(|m| m.as_str().to_string())
                        .filter(|s| !s.is_empty())
    ...
    async fn on_publish_requested(
        ...
        if state == SessionState::Pending {
            if let Err(e) = self.accept_request(request_id, ct).await {
                lifecycle.disconnect();
                anyhow::bail!(
