//! FLV tag types, encoding, and packetization.
//!
//! - `tag` — `FlvTag` enum (Audio/Video/ScriptData)
//! - `encode` — `encode_flv_header()` / `encode_flv_tag()` (pure Rust)
//! - `packetizer` — `FlvTagPacketizer` (FFmpeg-bound, FlvTag → Packet)
//! - `mapping` — `FlvStreamMapping` (stream index/timebase lookup)
...
impl TryFrom<Bytes> for FlvTag {
    ...
    fn try_from(data: Bytes) -> Result<Self, Self::Error> {
        // Expect one complete FLV tag block:
        //   header(11) + payload(data_size) + previous_tag_size(4)
        if data.len() < 15 {
